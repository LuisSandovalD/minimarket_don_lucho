...
export const saleSchema = z.object({
  ...
  items: z.array(z.object({ productId: z.string().min(1), quantity: z.coerce.number().positive(), unitPrice: z.coerce.number().positive(), discount: z.coerce.number().min(0).default(0) })).min(1).max(500).refine(items => new Set(items.map(item => item.productId)).size === items.length, "Agrupa cada producto en una sola línea."),
