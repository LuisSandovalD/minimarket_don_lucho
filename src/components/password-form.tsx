...
export function PasswordForm() {
    ...
    async function verify() {
        ...
        try {
            ...
            toast.error("No se pudo enviar el enlace de verificación.");
        } finally {
            setVerifying(false);
        }
    }
        ...
        <div className="w-full max-w-lg space-y-6">
            <form action={action} className="space-y-5">
                <div className="flex items-center gap-3 rounded-xl bg-muted/40 p-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                            Al cambiarla se cerrarán las sesiones activas.
                        </p>
                    </div>
                </div>
                ...
                {state && (
                    <p
                        role="status"
                        className={`rounded-lg px-3 py-2 text-sm ${state.success
                                ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400"
                                : "bg-destructive/10 text-destructive"
                            }`}
                    >
                        {state.success
                            ? "Contraseña cambiada. Inicia sesión nuevamente."
                            : state.message}
                    </p>
                )}
            ...
            <div className="flex items-center justify-between gap-3 rounded-xl bg-muted/40 p-4">
                <div>
                    <p className="text-sm font-medium">Verificación de correo</p>
                    <p className="text-xs text-muted-foreground">
                        Envía un nuevo enlace de verificación a tu correo.
                    </p>
                </div>
                ...
                >
                    ...
                    {verifying ? "Enviando..." : "Enviar enlace"}
                </Button>
            </div>
        </div>
    );
