...
export function CashForm({
    ...
    session?: { id: string; openingAmount: string } | null;
}) {
    const [state, action, pending] = useActionState(
        session ? closeCashAction : openCashAction,
        null
    );
    ...
    return (
        <form action={action} className="space-y-5">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-sm font-medium">
                            {session ? "Caja abierta" : "Apertura de caja"}
                        </p>

                        <p className="text-xs text-muted-foreground">
                            {session
                                ? "Registra el efectivo contado para cerrar la caja."
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        <div className="space-y-2">
                            ...
                            >
                                <SelectTrigger
                                    id="cashRegisterId"
                                    className="w-full border-0 bg-muted/40 shadow-sm"
                                >
            ...
            <Button
                type="submit"
                size="lg"
                disabled={pending || (!session && !registerId)}
