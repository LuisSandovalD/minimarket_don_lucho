...
export default async function Page({ params, searchParams }: { params: Promise<{ kind: string }>; searchParams: Promise<{ page?: string; q?: string }> }) {
  const { kind } = await params; if (!catalogConfig[kind]) notFound(); const config = catalogConfig[kind]; const actor = await requirePermission(config.view);
  const s = await searchParams, page = Math.max(1, Math.floor(Number(s.page) || 1)), q = (s.q ?? "").slice(0,100); const data = await listCatalog(kind, page, q);
  ...
  return <CatalogManager kind={kind} rows={JSON.parse(JSON.stringify(data.rows))} total={data.total} page={page} query={q} options={{ parentId: categories, roleId: roles }} permissions={permissions} canCreate={actor.permissions.includes(config.create)} canUpdate={actor.permissions.includes(config.update)}/>;
}
